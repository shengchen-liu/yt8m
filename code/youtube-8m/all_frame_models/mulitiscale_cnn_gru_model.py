...
class MultiscaleCnnGruModel(models.BaseModel):
  ...
  def rnn(self, model_input, gru_size, num_frames,
  ...
  def create_model(self, model_input, vocab_size, num_frames,
    ...
    num_layers = FLAGS.multiscale_cnn_gru_layers
    gru_size = int(FLAGS.gru_cells)
    pool_size=2
    ...
    for layer in range(num_layers):
      ...
      gru_memory = self.rnn(cnn_output_relu, gru_size, num_frames, sub_scope="rnn%d"%(layer+1))
      sub_prediction = self.moe(gru_memory, vocab_size, scopename="moe%d"%(layer+1))
